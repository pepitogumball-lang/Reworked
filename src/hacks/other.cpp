    ...
    void update(float dt) {
        ...
        if (g.speedhackEnabled && !g.frameStepper) {
            ...
            if (speedhackValue != "0.0" && speedhackValue != "") {
                float rawSpeed = std::stof(speedhackValue);
                rawSpeed = std::clamp(rawSpeed, 0.1f, 16.f);

                float decimals = rawSpeed - static_cast<int>(rawSpeed);
                ...
                speedhack = static_cast<int>(rawSpeed) + closest;
            }
        ...
        float cappedDt = std::min(dt * speedhack, 0.25f);
        CCScheduler::update(cappedDt);
    }
...
class $modify(PlayLayer) {
    ...
    void showNewBest(bool p0, int p1, int p2, bool p3, bool p4, bool p5) {
        ...
        if (g.blockOrbRewards) return;
        if ((g.safeMode || g.state != state::none) && autoSafe) return;
    ...
    void levelComplete() {
        ...
        if ((g.safeMode || g.state != state::none) && autoSafe)
            m_isTestMode = true;
...
class $modify(GJGameLevel) {
    ...
    void savePercentage(int p0, bool p1, int p2, int p3, bool p4) {
        auto& g = Global::get();
        bool autoSafe = Mod::get()->getSavedValue<bool>("macro_auto_safe_mode");
